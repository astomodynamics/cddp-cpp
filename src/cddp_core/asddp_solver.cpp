...
void ASDDPSolver::initialize(CDDP &context) {
  ...
  if (options.warm_start) {
    ...
    if (valid_warm_start) {
      if (options.verbose) {
        std::cout << "ASDDP: Using warm start with existing control gains"
                  << std::endl;
      }
      ...
      if (options.verbose) {
        std::cout << "ASDDP: Warning - warm start requested but no valid "
                     "solver state found. "
  ...
  while (iter < options.max_iterations) {
    ...
    if (options.max_cpu_time > 0) {
      ...
      if (duration.count() > options.max_cpu_time * 1000) {
        ...
        if (options.verbose) {
          std::cerr
              << "ASDDP: Maximum CPU time reached. Returning current solution"
              << std::endl;
        }
        break;
      }
    ...
    while (!backward_pass_success) {
      ...
      if (!backward_pass_success) {
        ...
        if (context.isRegularizationLimitReached()) {
          ...
          if (options.verbose) {
            std::cerr << "ASDDP: Backward pass regularization limit reached"
                      << std::endl;
          }
          break;
        }
    ...
    if (best_result.success) {
      ...
      context.merit_function_ =
          best_result.cost; // For ASDDP, merit function equals cost
      ...
      if (context.isRegularizationLimitReached()) {
        ...
        converged = false;
        if (options.verbose) {
          std::cerr << "ASDDP: Regularization limit reached. Not converged."
                    << std::endl;
        }
        break;
      }
...
std::string ASDDPSolver::getSolverName() const { return "ASDDP"; }
...
bool ASDDPSolver::backwardPass(CDDP &context) {
  ...
  for (int t = horizon - 1; t >= 0; --t) {
    ...
    if (es.eigenvalues().real().minCoeff() <= 0) {
      if (options.debug) {
        std::cerr << "ASDDP: Q_uu is not positive definite at time " << t
                  << std::endl;
      }
      return false;
    }
    ...
    if (control_box_constraint != nullptr) {
      for (int j = 0; j < control_dim; j++) {
        if (u(j) <=
            control_box_constraint->getLowerBound()(j) + active_set_tol) {
        ...
        } else if (u(j) >= control_box_constraint->getUpperBound()(j) -
                               active_set_tol) {
          Eigen::VectorXd e = Eigen::VectorXd::Zero(control_dim);
    ...
    if (active_constraint_index == 0) { // No active constraints
      ...
      Eigen::MatrixXd lambda =
          -(grad_u_g * Q_uu_inv * grad_u_g.transpose()).inverse() *
          (grad_u_g * Q_uu_inv * Q_u);
      ...
      Eigen::MatrixXd C_new =
          Eigen::MatrixXd::Zero(active_count_new, control_dim);
      Eigen::MatrixXd D_new =
          Eigen::MatrixXd::Zero(active_count_new, state_dim);
      ...
      if (active_count_new > 0) {
        ...
        Eigen::MatrixXd W = -(C_new * Q_uu_inv * C_new.transpose()).inverse() *
                            (C_new * Q_uu_inv);
        Eigen::MatrixXd H =
  ...
  if (!options.enable_parallel) {
    ...
    for (auto &future : futures) {
      try {
        ...
        if (options.verbose) {
          std::cerr << "ASDDP: Forward pass thread failed: " << e.what()
                    << std::endl;
        }
      }
    }
  ...
  for (int t = 0; t < context.getHorizon(); ++t) {
    ...
    if (t < context.getHorizon() - 1) {
      ...
      Eigen::VectorXd x_next =
          context.getSystem().getDiscreteDynamics(x, u, t * timestep);
    ...
    try {
      ...
      if (exit_code != osqp::OsqpExitCode::kOptimal) {
        if (options.debug) {
          std::cerr << "ASDDP: QP solver failed at time step " << t
                    << std::endl;
        }
        result.success = false;
        return result;
      ...
      if (options.debug) {
        std::cerr << "ASDDP: OSQP exception at time step " << t << ": "
                  << e.what() << std::endl;
      }
      result.success = false;
      return result;
  ...
  if (dJ <= 0) {
    if (options.debug) {
      std::cerr << "ASDDP: Forward pass did not yield sufficient decrease (dJ: "
                << dJ << ", reduction_ratio: " << reduction_ratio << ")"
                << std::endl;
    }
    result.success = false;
  } else {
...
void ASDDPSolver::printSolutionSummary(const CDDPSolution &solution) const {
  ...
  auto final_inf_pr =
      std::any_cast<double>(solution.at("final_primal_infeasibility"));

  std::cout << "Status: " << status << "\n";
  std::cout << "Iterations: " << iterations << "\n";
  std::cout << "Solve Time: " << std::setprecision(2) << solve_time << " ms\n";
  std::cout << "Final Cost: " << std::setprecision(6) << final_cost << "\n";
  std::cout << "Final Constraint Violation: " << std::setprecision(4)
            << final_inf_pr << "\n";
  std::cout << "========================================\n\n";
}
