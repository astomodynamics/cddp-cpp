...
namespace cddp
{
    ...
    void CDDP::initializeALDDP()
    {
        ...
        if (!system_)
        {
            ...
            if (options_.verbose)
            {
                std::cerr << "ALDDP::initializeALDDP: No dynamical system provided." << std::endl;
            }
            return;
        }
        ...
        if (!objective_)
        {
            ...
            if (options_.verbose)
            {
                std::cerr << "ALDDP::initializeALDDP: No objective function provided." << std::endl;
            }
            return;
        }
        ...
        {
            std::cerr << "ALDDP: Initial state and goal state in the objective function do not match" << std::endl;
    ...
    CDDPSolution CDDP::solveALDDP()
    {
        ...
        for (const auto &constraint_pair : constraint_set_)
        {
            ...
            for (int t = 0; t < horizon_; ++t)
            {
                Y_[constraint_name][t] = Eigen::VectorXd::Zero(dual_dim);
            }
        }
        ...
        //     {
        //         backward_pass_success = solveALDDPBackwardPass(Y_, rho);
        ...
        //         ForwardPassResult result = solveALDDPForwardPass(alpha, Y_, rho);
        ...
        //                     Y_[constraint_name][t] += rho * G_[constraint_name][t];
        //                 }
        //             }
