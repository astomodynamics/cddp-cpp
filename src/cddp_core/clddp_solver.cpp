...
void CLDDPSolver::initialize(CDDP &context) {
  ...
  // Cold start: full initialization (also used as fallback for invalid warm
  // start)
  k_u_.resize(horizon);
  K_u_.resize(horizon);
  for (int t = 0; t < horizon; ++t) {
...
CDDPSolution CLDDPSolver::solve(CDDP &context) {
  ...
  if (options.return_iteration_info) {
    // Pre-allocate history vectors for efficiency (max_iterations + 1 for
    // initial iteration)
  ...
  std::string termination_reason = "MaxIterationsReached"; // Default assumption
  ...
  while (iter < options.max_iterations) {
    ...
    if (options.max_cpu_time > 0) {
      ...
      if (duration.count() > options.max_cpu_time * 1000) {
        ...
        if (options.verbose) {
          std::cerr
              << "CLDDP: Maximum CPU time reached. Returning current solution"
              << std::endl;
        }
        break;
      }
    ...
    while (!backward_pass_success) {
      ...
      if (!backward_pass_success) {
        ...
        if (context.isRegularizationLimitReached()) {
          ...
          if (options.verbose) {
            std::cerr << "CLDDP: Backward pass regularization limit reached"
                      << std::endl;
          }
          break;
        }
    ...
    if (best_result.success) {
      ...
      if (options.return_iteration_info) {
        history_objective.push_back(context.cost_);
        history_merit_function.push_back(context.merit_function_);
        history_step_length_primal.push_back(
            context.getCurrentPrimalStepSize());
        history_dual_infeasibility.push_back(context.inf_du_);
        history_regularization.push_back(context.regularization_);
      }
      ...
      if (context.isRegularizationLimitReached()) {
        ...
        converged = false;
        if (options.verbose) {
          std::cerr << "CLDDP: Regularization limit reached. Not converged."
                    << std::endl;
        }
        break;
      }
  ...
  // Populate final solution with detailed status
  ...
  time_points.reserve(static_cast<size_t>(context.getHorizon() +
  ...
  if (!options.enable_parallel) {
    ...
    for (auto &future : futures) {
      try {
        ...
        if (options.verbose) {
          std::cerr << "CLDDP: Forward pass thread failed: " << e.what()
                    << std::endl;
        }
      }
    }
...
void CLDDPSolver::computeCost(CDDP &context) {
  ...
  context.merit_function_ =
      context.cost_; // For CLDDP, merit function equals cost
}

void CLDDPSolver::printIteration(int iter, double cost, double merit,
  ...
  std::cout << std::setw(4) << iter << " " << std::setw(12) << std::scientific
            << std::setprecision(4) << cost << " " << std::setw(10)
            << std::scientific << std::setprecision(2) << inf_du << " "
            << std::setw(8) << std::fixed << std::setprecision(1)
            << std::log10(regularization) << " " << std::setw(8) << std::fixed
            << std::setprecision(4) << alpha << std::endl;
}
