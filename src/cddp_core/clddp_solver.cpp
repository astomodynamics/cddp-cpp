...
namespace cddp
{
    ...
    CDDPSolution CLDDPSolver::solve(CDDP &context)
    {
        ...
        {
            // Pre-allocate history vectors for efficiency (max_iterations + 1 for initial iteration)
        ...
        time_points.reserve(static_cast<size_t>(context.getHorizon() + 1)); // Pre-allocate for efficiency
