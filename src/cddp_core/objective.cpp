...
QuadraticObjective::QuadraticObjective(
    ...
    const Eigen::MatrixXd &Qf, const Eigen::VectorXd &reference_state,
    const std::vector<Eigen::VectorXd> &reference_states, double timestep)
    : Q_(Q), R_(R), Qf_(Qf), reference_state_(reference_state),
      reference_states_(reference_states), timestep_(timestep) {
  ...
  if (reference_states_.size() > 0) {
    if ((reference_states_.back() - reference_state_).norm() > 1e-6) {
      std::cerr << "QuadraticObjective: Last reference state must be same as "
                   "the reference state"
                << std::endl;
      throw std::invalid_argument(
          "Last reference state must be same as the reference state");
    }
  }
}

// Evaluate the total cost: terminal cost + running cost
double QuadraticObjective::evaluate(
...
double QuadraticObjective::running_cost(const Eigen::VectorXd &state,
  ...
  return ((state_error.transpose() * Q_ * state_error).value() +
          (control.transpose() * R_ * control).value());
}

// Evaluate the final/terminal cost: (x_T - x_ref)^T Qf (x_T - x_ref)
double
QuadraticObjective::terminal_cost(const Eigen::VectorXd &final_state) const {
...
Eigen::VectorXd
QuadraticObjective::getRunningCostStateGradient(const Eigen::VectorXd &state,
                                                const Eigen::VectorXd &control,
                                                int index) const {
  Eigen::VectorXd state_error;
  if (reference_states_.size() > 0) {
...
Eigen::VectorXd QuadraticObjective::getRunningCostControlGradient(
    const Eigen::VectorXd &state, const Eigen::VectorXd &control,
    int index) const {
  return 2.0 * R_ * control;
}

// Gradient of the final cost w.r.t state
Eigen::VectorXd QuadraticObjective::getFinalCostGradient(
...
Eigen::MatrixXd
QuadraticObjective::getRunningCostStateHessian(const Eigen::VectorXd &state,
                                               const Eigen::VectorXd &control,
                                               int index) const {
  return 2.0 * Q_;
}
...
Eigen::MatrixXd
QuadraticObjective::getRunningCostControlHessian(const Eigen::VectorXd &state,
                                                 const Eigen::VectorXd &control,
                                                 int index) const {
  return 2.0 * R_;
}
...
Eigen::MatrixXd
QuadraticObjective::getRunningCostCrossHessian(const Eigen::VectorXd &state,
                                               const Eigen::VectorXd &control,
                                               int index) const {
...
Eigen::MatrixXd QuadraticObjective::getFinalCostHessian(
    const Eigen::VectorXd &final_state) const {
  return 2.0 * Qf_;
}

NonlinearObjective::NonlinearObjective(double timestep) : timestep_(timestep) {}
...
double NonlinearObjective::running_cost(const Eigen::VectorXd &state,
                                        const Eigen::VectorXd &control,
                                        int index) const {
...
double
NonlinearObjective::terminal_cost(const Eigen::VectorXd &final_state) const {
  // Derived classes should override this
  return 0.0;
}

Eigen::VectorXd
NonlinearObjective::getRunningCostStateGradient(const Eigen::VectorXd &state,
                                                const Eigen::VectorXd &control,
                                                int index) const {
...
Eigen::MatrixXd
NonlinearObjective::getRunningCostStateHessian(const Eigen::VectorXd &state,
                                               const Eigen::VectorXd &control,
                                               int index) const {
...
Eigen::MatrixXd
NonlinearObjective::getRunningCostControlHessian(const Eigen::VectorXd &state,
                                                 const Eigen::VectorXd &control,
                                                 int index) const {
...
Eigen::MatrixXd
NonlinearObjective::getRunningCostCrossHessian(const Eigen::VectorXd &state,
                                               const Eigen::VectorXd &control,
                                               int index) const {
