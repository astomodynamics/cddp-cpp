...
namespace cddp
{
    ...
    CDDPSolution CDDP::solveLogCDDP()
    {
        ...
        while (iter < options_.max_iterations) {
            ...
            if (backward_pass_success) {
                forward_pass_success = solveLogCDDPForwardPass();
            }
...
bool CDDP::solveLogCDDPForwardPass()
{
    ...
    for (int iter = 0; iter < options_.max_line_search_iterations; ++iter) {
        std::cout << "Line search iteration: " << iter << " with alpha = " << alpha << std::endl;
        double J_new = 0.0;
        double L_new = 0.0;
        ...
        if (!is_feasible) {
            if (options_.verbose) {
                std::cout << "Infeasible solution with alpha = " << alpha << std::endl;
            }
            alpha *= options_.backtracking_factor;

            if (alpha < options_.backtracking_min) {
                std::cout << "Line search failed: alpha too small" << std::endl;
                return false; // Exit if alpha is too small
            }
            continue;
        }

        // If we have a positive expected improvement, check actual improvement ratio
        if (expected > 0) {
            ...
            if (ratio > options_.minimum_reduction_ratio) {
                ...
                success = true;
                if (options_.verbose) {
                    std::cout << "Line search success with alpha = " << alpha << std::endl;
                }
