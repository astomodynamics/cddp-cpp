...
public:
    ...
    virtual std::tuple<Eigen::MatrixXd, Eigen::MatrixXd> getJacobians(const Eigen::VectorXd& state, 
        ...
        return {getStateJacobian(state, control, time), getControlJacobian(state, control, time)};
    }
...
protected:
    ...
    Eigen::VectorXd euler_step(const Eigen::VectorXd& state, const Eigen::VectorXd& control, double dt, double time) const;
    Eigen::VectorXd heun_step(const Eigen::VectorXd& state, const Eigen::VectorXd& control, double dt, double time) const;
    Eigen::VectorXd rk3_step(const Eigen::VectorXd& state, const Eigen::VectorXd& control, double dt, double time) const;
    Eigen::VectorXd rk4_step(const Eigen::VectorXd& state, const Eigen::VectorXd& control, double dt, double time) const;
};
}
