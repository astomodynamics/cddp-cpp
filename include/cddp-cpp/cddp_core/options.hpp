...
namespace cddp
{
     ...
     *
     * These parameters control how the step size (alpha) is determined during the forward pass
     * to ensure sufficient decrease in the cost or merit function.
     */
    struct LineSearchOptions
    {
        int max_iterations = 11;            ///< Maximum number of backtracking steps in line search.
        double initial_step_size = 1.0;     ///< Initial step size (alpha) for line search.
        double min_step_size = 1e-8;        ///< Minimum allowed step size in line search.
        double step_reduction_factor = 0.5; ///< Factor by which to reduce alpha at each backtracking iteration.
    };

    /**
     * @brief Options for the regularization scheme.
     *
     * Regularization is used to ensure that the Hessian matrices in the backward pass are
     * positive definite, improving numerical stability.
     */
     ...
     * This struct will be instantiated within each solver's specific options if it uses a barrier method.
     */
    ...
    /**
     * @brief Options for the filter line search mechanism within an Interior Point Method.
     * This struct will be instantiated within each solver's specific options if it uses a filter line search.
     */
    struct SolverSpecificFilterOptions
    {
        double merit_acceptance_threshold = 1e-6;     ///< Small value for merit filter acceptance.
        double violation_acceptance_threshold = 1e-6; ///< Small value for violation filter acceptance.
        double max_violation_threshold = 1e+4;        ///< Maximum violation for filter acceptance.
        double min_violation_for_armijo_check = 1e-7; ///< Min violation below which Armijo is checked on cost.
        double armijo_constant = 1e-4;                ///< Armijo constant (c1) for filter acceptance.
    };

    /**
     * @brief Options for the relaxed log-barrier method.
     * This struct will be instantiated within each solver's specific options if it uses a relaxed log-barrier method.
     */
    ...
    /**
     * @brief Comprehensive options specifically for the IPDDP (Interior-Point Differential Dynamic Programming) algorithm.
     * Includes its own instances of barrier and filter parameters.
     */
    ...
    /**
     * @brief Comprehensive options specifically for the MSIPDDP (Multi-Shooting Interior-Point Differential Dynamic Programming) algorithm.
     * Includes its own instances of barrier and filter parameters, plus multi-shooting specifics.
     */
    ...
    struct CDDPOptions
    {
        ...
        bool print_solver_header_footer = true;        ///< Print header and footer summary for the solve.
        bool use_ilqr = true;                          ///< Use iLQR (ignore second-order dynamics derivatives).
        bool enable_parallel = false;                  ///< Enable parallel computation for line search.
