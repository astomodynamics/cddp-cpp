...
/**
 * @brief Augmented Lagrangian Differential Dynamic Programming (ALDDP) solver
 * implementation.
 *
...
private:
  ...
  std::vector<Eigen::VectorXd>
      Lambda_; ///< Lagrange multipliers for defect constraints

  // Penalty parameters (simplified for ALDDP)
  double
      rho_defect_; ///< Defect constraint penalty parameter (scalar, constant)
  std::map<std::string, double>
      rho_path_; ///< Path constraint penalty parameters (constraint name ->
                 ///< scalar)

  double cost_;                 ///< Current total cost
  double constraint_violation_; ///< Current constraint violation measure
  double
      lagrangian_value_;  ///< Augmented Lagrangian value (cost + penalty terms)
  double optimality_gap_; ///< Norm of the gradient of the Lagrangian

  /**
   * @brief Evaluate the trajectory, computing cost, dynamics, and augmented
   * lagrangian.
  ...
  /**
   * @brief Perform the backward pass (Riccati recursion with augmented
   * Lagrangian terms).
   * @param context Reference to the CDDP context.
  ...
  void printIteration(int iter, double cost, double lagrangian,
                      double grad_norm, double regularization, double alpha,
                      double mu, double constraint_violation) const;
