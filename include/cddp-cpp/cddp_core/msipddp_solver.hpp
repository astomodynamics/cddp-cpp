...
/**
 * @brief Multi-Shooting Interior Point Differential Dynamic Programming
 * (MSIPDDP) solver implementation.
 *
 * This class implements the ISolverAlgorithm interface to provide
 * a multi-shooting interior point method based DDP solver for handling
 * inequality constraints. Uses primal-dual interior point method with
 * logarithmic barrier and costate variables for gap-closing between shooting
 * segments.
 */
...
private:
  ...
  std::map<std::string, std::vector<Eigen::MatrixXd>>
      G_x_; ///< Constraint state jacobians
  std::map<std::string, std::vector<Eigen::MatrixXd>>
      G_u_; ///< Constraint control jacobians
  ...
  std::map<std::string, std::vector<Eigen::VectorXd>>
      Y_; ///< Dual variables (Lagrange multipliers)
  std::map<std::string, std::vector<Eigen::VectorXd>> S_; ///< Slack variables

  // MSIPDDP gains for dual and slack variables
  std::map<std::string, std::vector<Eigen::VectorXd>>
      k_y_; ///< Feedforward gains for dual variables
  std::map<std::string, std::vector<Eigen::MatrixXd>>
      K_y_; ///< Feedback gains for dual variables
  std::map<std::string, std::vector<Eigen::VectorXd>>
      k_s_; ///< Feedforward gains for slack variables
  std::map<std::string, std::vector<Eigen::MatrixXd>>
      K_s_; ///< Feedback gains for slack variables
  ...
  /**
   * @brief Pre-compute constraint jacobians for all time steps and constraints
   * in parallel.
   * @param context Reference to the CDDP context.
   */
  ...
  /**
   * @brief Evaluate trajectory for warm start without overwriting the state
   * trajectory.
   * @param context Reference to the CDDP context.
   */
  ...
  /**
   * @brief Initialize dual, slack, and costate variables for the MSIPDDP
   * algorithm.
   * @param context Reference to the CDDP context.
   */
  ...
  /**
   * @brief Perform backward pass (primal-dual Riccati recursion with costate
   * variables).
   * @param context Reference to the CDDP context.
  ...
  /**
   * @brief Perform single forward pass with given step size and multi-shooting
   * rollout.
   * @param context Reference to the CDDP context.
