...
public:
  ...
  virtual double
  evaluate(const std::vector<Eigen::VectorXd> &states,
           const std::vector<Eigen::VectorXd> &controls) const = 0;
  ...
  getRunningCostGradients(const Eigen::VectorXd &state,
                          const Eigen::VectorXd &control, int index) const {
    return {getRunningCostStateGradient(state, control, index),
            getRunningCostControlGradient(state, control, index)};
  }

  // Gradient of the final cost w.r.t state: dlf/dx
  virtual Eigen::VectorXd
  getFinalCostGradient(const Eigen::VectorXd &final_state) const = 0;
  ...
  getRunningCostHessians(const Eigen::VectorXd &state,
                         const Eigen::VectorXd &control, int index) const {
    return {getRunningCostStateHessian(state, control, index),
            getRunningCostControlHessian(state, control, index),
            getRunningCostCrossHessian(state, control, index)};
  }

  // Hessian of the final cost w.r.t state: d^2lf/dx^2
  virtual Eigen::MatrixXd
  getFinalCostHessian(const Eigen::VectorXd &final_state) const = 0;

  // Accessors
  virtual Eigen::VectorXd getReferenceState() const { return reference_state_; }
  ...
  virtual void
  setReferenceStates(const std::vector<Eigen::VectorXd> &reference_states) {
...
public:
  ...
  QuadraticObjective(
      ...
      const Eigen::VectorXd &reference_state =
          Eigen::VectorXd::Zero(0), // Default to empty vector
      const std::vector<Eigen::VectorXd> &reference_states =
          std::vector<Eigen::VectorXd>(), // Default to empty vector
      double timestep = 0.1);
  ...
  double running_cost(const Eigen::VectorXd &state,
                      const Eigen::VectorXd &control, int index) const override;
...
protected:
  Eigen::MatrixXd Q_, R_,
      Qf_; // Weight matrices for state, control, and final state
 ...
 *
 * This class implements a general nonlinear objective function with finite
 * difference methods for computing gradients and Hessians. It extends the base
 * Objective class and provides utilities for numerical differentiation.
 */
