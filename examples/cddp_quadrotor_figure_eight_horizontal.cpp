...
int main()
{
    ...
    // Plot the XY trajectory (top-down view of figure-eight)
    figure();
    plot(x_arr, y_arr, "b-")->line_width(2);
    xlabel("X Position (m)");
    ylabel("Y Position (m)");
    title("Quadrotor Figure-Eight Trajectory (XY Plane)");
    grid(true);
    
    // Save the XY trajectory plot
    save(plotDirectory + "/quadrotor_figure_eight_xy.png");
