...
int main() {
    ...
    // -------------------------------------------------
    // 2. Configure and Solve IPDDP with Multiple Constraints
    // -------------------------------------------------
    ...
    // Create points for the linear constraint
    std::vector<double> x_linear, y_linear;
    for (double x = -1.0; x <= 4.0; x += 0.1) {
        double y = 1.0 * x + 0.5;
        x_linear.push_back(x);
        y_linear.push_back(y);
    }
    ...
    // Linear constraint
    auto linear_constraint = plot(ax, x_linear, y_linear);
    linear_constraint->line_style("--");
    linear_constraint->line_width(2);
    linear_constraint->color("red");
    linear_constraint->display_name("Linear Constraint");
    ...
    std::cout << "Trajectory comparison saved to "
              << (plotDirectory + "/trajectory_comparison_ipddp_v3_linear_matplot.png")
              << std::endl;
