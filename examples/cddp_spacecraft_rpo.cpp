...
namespace cddp
{
    ...
    public:
        ...
        {
            // Running cost does not depend on state after user modifications
            if (goal_state_.size() > 0)
            {                                                     // Check if state dimension is available
                return Eigen::VectorXd::Zero(goal_state_.size()); // Assuming state has same dim as goal_state_
            }
            return Eigen::VectorXd::Zero(0); // Or handle error appropriately if size unknown
        }
        ...
        {
            // Running cost does not depend on state
            if (goal_state_.size() > 0)
            {
                return Eigen::MatrixXd::Zero(goal_state_.size(), goal_state_.size());
            }
        ...
        Eigen::MatrixXd getRunningCostControlHessian(const Eigen::VectorXd & /*state*/, const Eigen::VectorXd &control, int /*index*/) const override
        {
            ...
            if (norm_u < 1e-8)
            {
                // Return a well-conditioned approximation, e.g., scaled identity
                // Or simply zero if controls are expected to be exactly zero without penalty change
        ...
        {
            // No cross-dependency between state and control in running cost
            if (goal_state_.size() > 0 && control.size() > 0)
            {
                return Eigen::MatrixXd::Zero(control.size(), goal_state_.size());
            }
            return Eigen::MatrixXd::Zero(0, 0);
        }
