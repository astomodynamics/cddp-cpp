...
//     // Solve the problem
//     cddp::CDDPSolution solution = cddp_solver.solve("LogDDP");
//     // cddp::CDDPSolution solution = cddp_solver.solveLogDDP();
...
TEST(LogDDPTest, SolveCar)
{
    ...
    // Create CDDP Options
    cddp::CDDPOptions options;
    options.max_iterations = 1000;         // Reasonable number for testing
...
TEST(LogDDPTest, SolveQuadrotor)
{
    ...
    // Verify quadrotor moved towards the goal
    Eigen::Vector3d initial_position = initial_state.head(3);
    double initial_distance = (initial_position - goal_position).norm();
    EXPECT_LT(position_error, initial_distance) << "Quadrotor should move closer to the goal";
    ...
    cddp::CDDPOptions warm_options = options;
    warm_options.warm_start = true;
    warm_options.max_iterations = 5; // Fewer iterations for warm start
    warm_options.verbose = false;    // Less verbose for warm start test
