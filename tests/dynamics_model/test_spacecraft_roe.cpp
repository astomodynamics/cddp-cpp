...
TEST(TestSpacecraftROE, RelativeTrajectory)
{
    ...
    for (int i = 0; i < num_steps; ++i) {
        double t = i * dt;
        current_roe_state = qnsRoeModel.getDiscreteDynamics(current_roe_state, control, t);
        roe_trajectory.push_back(current_roe_state);
...
TEST(TestSpacecraftROE, JacobianBasedPropagation)
{
    ...
    {
        x_curr_dyn_zero_u = qnsRoeModel.getDiscreteDynamics(x_curr_dyn_zero_u, u_zero_control, k * dt);
        x_trajectory_dyn_zero_u.push_back(x_curr_dyn_zero_u);

        Eigen::MatrixXd A = qnsRoeModel.getStateJacobian(x_curr_lin_zero_u, u_zero_control, k * dt);
        Eigen::MatrixXd B = qnsRoeModel.getControlJacobian(x_curr_lin_zero_u, u_zero_control, k * dt);
    ...
    for (int k = 0; k < num_steps; ++k)
    {
        x_curr_dyn_nonzero_u = qnsRoeModel.getDiscreteDynamics(x_curr_dyn_nonzero_u, u_nonzero_control, k * dt);
        x_trajectory_dyn_nonzero_u.push_back(x_curr_dyn_nonzero_u);

        Eigen::MatrixXd A = qnsRoeModel.getStateJacobian(x_curr_lin_nonzero_u, u_nonzero_control, k * dt);
        Eigen::MatrixXd B = qnsRoeModel.getControlJacobian(x_curr_lin_nonzero_u, u_nonzero_control, k * dt);
        x_curr_lin_nonzero_u = x_curr_lin_nonzero_u + dt * (A * x_curr_lin_nonzero_u + B * u_nonzero_control);
        x_trajectory_lin_nonzero_u.push_back(x_curr_lin_nonzero_u);
    }
